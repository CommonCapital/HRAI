...
export function AccountManager({
    ...
    const handleConnect = async () => {
        // Trigger Google OAuth via better-auth
        await authClient.signIn.social({
            provider: "google",
            callbackURL: "/scheduling",
        });
    };
    ...
    const handleDisconnect = async (accountKey: string) => {
        ...
        // TODO: Implement trpc.scheduling.disconnectAccount
        setPendingAction(null);
    };

    const handleSetDefault = async (accountKey: string) => {
        setPendingAction(`default-${accountKey}`);
        // TODO: Implement trpc.scheduling.setDefaultAccount
        setPendingAction(null);
    };
                            ...
                            <div className="flex items-center gap-2">
                                {!account.isDefault && (
                                    <Button
                                        variant="outline"
                                        size="sm"
                                        onClick={() => handleSetDefault(account.id || account._key)}
                                        disabled={isPending}
                                    >
                                        {pendingAction === `default-${account.id || account._key}` ? (
                                ...
                                <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => handleDisconnect(account.id || account._key)}
                                    disabled={isPending}
                                    className="text-red-600 hover:text-red-700 hover:bg-red-50"
                                >
                                    {pendingAction === `disconnect-${account.id || account._key}` ? (
                                        <Loader2 className="h-4 w-4 animate-spin" />
                                    ) : (
                                        <>
