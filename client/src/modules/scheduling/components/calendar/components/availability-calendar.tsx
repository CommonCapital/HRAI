...
export function AvailabilityCalendar({
    ...
    const saveMutation = useMutation(
        trpc.scheduling.updateAvailability.mutationOptions({
            onSuccess: async () => {
                await queryClient.invalidateQueries({
                    queryKey: trpc.scheduling.getAvailability.queryKey(),
                });
            }
        })
    );
    ...
    const handleSave = async () => {
        ...
        // Convert Date objects to recurring slot format (dayOfWeek, startTime, endTime)
        const slots = blocksToSave.map(block => ({
            dayOfWeek: format(block.start, "EEEE").toLowerCase(),
        ...
        try {
            ...
            // After save, we probably want to sync the IDs, but since we use destructive delete/insert 
            // in the router for recurring slots, we can just mark as saved.
            // In a better implementation, we'd fetch the new list and call markAsSaved.
            const result = await queryClient.fetchQuery(trpc.scheduling.getAvailability.queryOptions());
            const newBlocks: TimeBlock[] = result.map((s: { id: string; dayOfWeek: string; startTime: string; endTime: string }) => {
                // We need to map dayOfWeek + time back to Dates in the current week view
                // For simplicity, let's just trigger a reload or rely on getAvailability query.
                // But markAsSaved is needed to clear 'hasChanges'
                return {
                    id: s.id,
                    start: new Date(), // This logic is tricky without knowing which day/week we are in
                    end: new Date()
                };
            });
            // Actually, markAsSaved(events) might be enough if we just want to reset the 'hasChanges' flag
            markAsSaved(events);
        } catch (error) {
